...
const GLOBAL_CSS = css`
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  input, textarea, select { font-family: ${T.fontBody}; outline: none; transition: border-color .2s, box-shadow .2s; }
  input:focus, textarea:focus, select:focus { border-color: ${T.ouro} !important; box-shadow: 0 0 0 3px ${T.ouro}28; }
...
function Hero({ onChamarRegisto, onIrParaLogin }) {
  return (
    <div style={{ position:"relative", overflow:"hidden", minHeight:560, display:"flex", flexDirection:"column" }}>
      ...
      <svg style={{ position:"absolute", inset:0, width:"100%", height:"100%", opacity:.07 }} viewBox="0 0 400 560" preserveAspectRatio="xMidYMid slice">
        {[...Array(6)].map((_,r) =>
          [...Array(8)].map((_,c) => (
            <polygon key={`${r}-${c}`} points={`${c*55+27},${r*90+5} ${c*55+52},${r*90+48} ${c*55+2},${r*90+48}`} fill={r%2===c%2 ? T.ouro : T.areia} />
          ))
        )}
      </svg>
      ...
      <nav style={{ position:"relative", zIndex:10, display:"flex", justifyContent:"space-between", alignItems:"center", padding:"20px 22px" }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={onIrParaLogin} style={{ background:"rgba(255,255,255,.1)", color:T.branco, border:"1px solid rgba(255,255,255,.2)", borderRadius:30, padding:"8px 14px", fontWeight:700, fontSize:12, backdropFilter:"blur(8px)" }}>
            🔒 Entrar
          </button>
          <button onClick={onChamarRegisto} style={{ background:`linear-gradient(135deg,${T.ouro},${T.ouroVivo})`, color:T.terra, border:"none", borderRadius:30, padding:"9px 18px", fontWeight:900, fontSize:12.5, boxShadow:`0 4px 16px ${T.ouro}50` }}>
            Tornar Sócio
          </button>
        </div>
      </nav>
...
function SobreTambato() {
  ...
  return (
    <section id="sobre" style={{ background:T.creme, padding:"50px 22px" }}>
      <TituloSeccao tag="Sobre Tambató" titulo="A Nossa Terra" subtitulo="Conhece a localização, as fronteiras e a história da tabanca de Tambató, no coração da Região de Oio." />
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            {fronteiras.map((f) => (
              <div key={f.dir} style={{ background:`rgba(255,255,255,.08)`, borderRadius:12, padding:"12px 14px", border:`1px solid ${f.tipo==="marítima" ? T.aguaMar : T.ouro}44` }}>
                ...
                <div style={{ fontSize:10, color:f.tipo==="marítima" ? "#7DD3FC" : T.palha, fontWeight:800, letterSpacing:1, textTransform:"uppercase" }}>{f.dir} · {f.tipo}</div>
                <div style={{ fontSize:13.5, fontWeight:700, color:T.branco, marginTop:2, fontFamily:T.fontDisplay }}>{f.local}</div>
              </div>
            ))}
          </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:12, marginBottom:30 }}>
        {INFO_AFAT.objetivos.map((obj, i) => (
          <div key={obj.titulo} className="animate-up" style={{ animationDelay:`${i*.08}s`, background:T.branco, borderRadius:14, padding:16, display:"flex", gap:14, alignItems:"flex-start", boxShadow:`0 2px 14px rgba(44,24,16,.06)`, borderLeft:`4px solid ${T.ouro}` }}>
            <div style={{ width:44, height:44, background:`${T.ouro}18`, borderRadius:12, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>{obj.icone}</div>
            <div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {INFO_AFAT.estatutos_resumo.map((est, i) => (
          <div key={est.art} style={{ background:T.branco, borderRadius:12, overflow:"hidden", boxShadow:`0 2px 10px rgba(44,24,16,.05)` }}>
            <button onClick={() => setAbaStat(abaStat === i ? -1 : i)} style={{ width:"100%", background:"none", border:"none", padding:"14px 16px", display:"flex", justifyContent:"space-between", alignItems:"center", cursor:"pointer", textAlign:"left" }}>
...
const PAISES_DIASPORA = ["Guiné-Bissau","Portugal","França","Espanha","Senegal","Gâmbia","Brasil","Estados Unidos","Angola","Cabo Verde","Outro"];

function FormularioSocio({ onSucesso }) {
  const [passo, setPasso]       = useState(1);
  const [dados, setDados]       = useState({ nome_completo:"", data_nascimento:"", genero:"", telefone:"", whatsapp:"", email:"", pais_residencia:"Guiné-Bissau", cidade_residencia:"", naturalidade:"Tambató", filiacao:"", foto:null, fotoPreview:null });
  const [erros, setErros]       = useState({});
  const [enviando, setEnviando] = useState(false);
  const inputFotoRef            = useRef();

  const atualizar = (campo, valor) => { setDados(p => ({ ...p, [campo]: valor })); setErros(p => ({ ...p, [campo]: null })); };
  const aoSelecionarFoto = async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) { setErros(p => ({ ...p, foto: "A foto não pode exceder 5 MB." })); return; }
  ...
  const validar = () => {
    const e = {};
    if (passo === 1) { if (!dados.nome_completo.trim()) e.nome_completo = "Nome é obrigatório"; if (!dados.data_nascimento) e.data_nascimento = "Data de nascimento é obrigatória"; if (!dados.genero) e.genero = "Indique o género"; }
    if (passo === 2) { if (!dados.telefone.trim()) e.telefone = "Contacto telefónico é obrigatório"; if (!dados.pais_residencia) e.pais_residencia = "Indique o país de residência"; }
    setErros(e);
  ...
  const submeter = async () => {
    ...
    setEnviando(true);
    try {
      const pedido = await submeterPedidoSocio({ nome_completo:dados.nome_completo, data_nascimento:dados.data_nascimento, genero:dados.genero, telefone:dados.telefone, whatsapp:dados.whatsapp||dados.telefone, email:dados.email, pais_residencia:dados.pais_residencia, cidade_residencia:dados.cidade_residencia, naturalidade:dados.naturalidade, filiacao:dados.filiacao, foto:dados.foto });
      onSucesso(pedido);
    ...
    <div style={{ display:"flex", justifyContent:"center", gap:6, marginBottom:24 }}>
      {[1,2,3].map(n => (
        <div key={n} style={{ display:"flex", alignItems:"center", gap:6 }}>
          <div style={{ width:32, height:32, borderRadius:"50%", background: passo>=n ? T.ouro : T.areia, color: passo>=n ? T.terra : T.muted, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:900, fontSize:13, transition:"all .3s" }}>{passo > n ? "✓" : n}</div>
  ...
  return (
    <div style={{ background:`${T.terra}0D`, borderRadius:18, padding:"0 0 8px", border:`1px solid ${T.areia}` }}>
      <div style={{ background:`linear-gradient(135deg,${T.terra},${T.argila})`, borderRadius:"18px 18px 0 0", padding:"22px 22px 18px" }}>
        ...
        <h3 style={{ fontFamily:T.fontDisplay, color:T.branco, fontSize:20, fontWeight:900, marginBottom:4 }}>Pedido de Cartão de Sócio</h3>
        <p style={{ color:"rgba(255,255,255,.6)", fontSize:12.5, lineHeight:1.55 }}>Preenche os teus dados. O pedido será analisado pela Direcção da AFAT.</p>
      </div>
      <div style={{ padding:"20px 18px 8px" }}>
        <PassoIndicador />
        {passo === 1 && (
          <div className="animate-in">
            <div style={{ fontFamily:T.fontDisplay, fontSize:16, fontWeight:700, color:T.terra, marginBottom:18 }}>👤 Dados Pessoais</div>
            <Campo label="Nome Completo" obrigatorio erro={erros.nome_completo}><input style={estiloInput} placeholder="O teu nome completo..." value={dados.nome_completo} onChange={e => atualizar("nome_completo", e.target.value)} /></Campo>
            <Campo label="Filiação (Nome do Pai / Mãe)" dica="Ajuda a distinguir homónimos"><input style={estiloInput} placeholder="Ex: Filho de Mamadu Baldé..." value={dados.filiacao} onChange={e => atualizar("filiacao", e.target.value)} /></Campo>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <Campo label="Data de Nascimento" obrigatorio erro={erros.data_nascimento}><input type="date" style={estiloInput} value={dados.data_nascimento} onChange={e => atualizar("data_nascimento", e.target.value)} /></Campo>
              <Campo label="Género" obrigatorio erro={erros.genero}><select style={estiloInput} value={dados.genero} onChange={e => atualizar("genero", e.target.value)}><option value="">Selecionar...</option><option value="M">Masculino</option><option value="F">Feminino</option><option value="N/D">Prefiro não dizer</option></select></Campo>
            </div>
            <Campo label="Naturalidade" dica="Localidade onde nasceste"><input style={estiloInput} placeholder="Ex: Tambató, Mansabã..." value={dados.naturalidade} onChange={e => atualizar("naturalidade", e.target.value)} /></Campo>
            <button onClick={avancar} style={{ width:"100%", background:`linear-gradient(135deg,${T.ouro},${T.ouroVivo})`, color:T.terra, border:"none", borderRadius:12, padding:"14px", fontWeight:900, fontSize:15, marginTop:4 }}>Próximo: Contacto →</button>
          </div>
        )}
        {passo === 2 && (
          <div className="animate-in">
            <div style={{ fontFamily:T.fontDisplay, fontSize:16, fontWeight:700, color:T.terra, marginBottom:18 }}>📞 Contacto & Residência</div>
            <Campo label="Número de Telefone" obrigatorio erro={erros.telefone} dica="Número local ou internacional"><input type="tel" style={estiloInput} placeholder="+245 9XX XXX XXX" value={dados.telefone} onChange={e => atualizar("telefone", e.target.value)} /></Campo>
            <Campo label="WhatsApp" dica="Se for diferente do telefone acima"><input type="tel" style={estiloInput} placeholder="+245 9XX XXX XXX" value={dados.whatsapp} onChange={e => atualizar("whatsapp", e.target.value)} /></Campo>
            <Campo label="Email (opcional)"><input type="email" style={estiloInput} placeholder="o.teu@email.com" value={dados.email} onChange={e => atualizar("email", e.target.value)} /></Campo>
            <Campo label="País de Residência" obrigatorio erro={erros.pais_residencia}><select style={estiloInput} value={dados.pais_residencia} onChange={e => atualizar("pais_residencia", e.target.value)}>{PAISES_DIASPORA.map(p => <option key={p} value={p}>{p}</option>)}</select></Campo>
            <Campo label="Cidade / Região de Residência"><input style={estiloInput} placeholder="Ex: Bissau, Lisboa, Paris..." value={dados.cidade_residencia} onChange={e => atualizar("cidade_residencia", e.target.value)} /></Campo>
            {dados.pais_residencia !== "Guiné-Bissau" && (
              <div style={{ background:`${T.aguaMar}18`, border:`1px solid ${T.aguaMar}44`, borderRadius:10, padding:"10px 14px", marginBottom:14, fontSize:12.5, color:T.aguaMar, fontWeight:700 }}>🌍 Membro da Diáspora — Bem-vindo!</div>
            )}
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        ...
        {passo === 3 && (
          <div className="animate-in">
            <div style={{ fontFamily:T.fontDisplay, fontSize:16, fontWeight:700, color:T.terra, marginBottom:18 }}>📷 Foto para o Cartão</div>
            <Campo label="Foto Tipo Passe" erro={erros.foto} dica="JPEG/PNG · Até 5 MB · Fundo neutro recomendado">
              <div onClick={() => inputFotoRef.current?.click()} style={{ border:`2px dashed ${dados.fotoPreview ? T.ouro : T.areia}`, borderRadius:14, padding:dados.fotoPreview ? 0 : "28px 20px", textAlign:"center", cursor:"pointer", overflow:"hidden", background: dados.fotoPreview ? "none" : T.branco }}>
                {dados.fotoPreview ? (
                  <div style={{ position:"relative" }}>
                    <img src={dados.fotoPreview} alt="Pré-visualização" style={{ width:"100%", maxHeight:240, objectFit:"cover", display:"block" }} />
                    <div style={{ position:"absolute", bottom:8, right:8, background:"rgba(0,0,0,.6)", color:T.branco, borderRadius:8, padding:"4px 10px", fontSize:11, fontWeight:700 }}>Toca para alterar</div>
                  </div>
                ) : (<><div style={{ fontSize:36, marginBottom:8 }}>📷</div><div style={{ fontWeight:700, color:T.terra, fontSize:14, marginBottom:4 }}>Adicionar Foto</div><div style={{ fontSize:12, color:T.muted }}>Toca aqui para escolher do telemóvel</div></>)}
              </div>
            ...
            <div style={{ background:T.branco, borderRadius:14, padding:16, marginBottom:18, border:`1px solid ${T.areia}` }}>
              ...
              {[["Nome",dados.nome_completo],["Nascimento",dados.data_nascimento ? new Date(dados.data_nascimento+"T00:00:00").toLocaleDateString("pt-PT") : "—"],["Género",dados.genero==="M"?"Masculino":dados.genero==="F"?"Feminino":dados.genero],["Telefone",dados.telefone],["Residência",`${dados.cidade_residencia||"—"} · ${dados.pais_residencia}`]].map(([l,v]) => (
            ...
            {erros.global && <div style={{ background:`${T.err}18`, color:T.err, border:`1px solid ${T.err}44`, borderRadius:10, padding:"10px 14px", marginBottom:12, fontSize:13, fontWeight:700 }}>⚠ {erros.global}</div>}
...
function EcranSucesso({ pedido, onPartilhar, onNovoPedido }) {
  return (
    <div className="animate-up" style={{ textAlign:"center", padding:"10px 0 16px" }}>
      ...
      <h3 style={{ fontFamily:T.fontDisplay, fontSize:22, fontWeight:900, color:T.terra, marginBottom:10 }}>Pedido Enviado com Sucesso!</h3>
      <div style={{ background:`${T.verdeClaro}15`, border:`1px solid ${T.verdeClaro}44`, borderRadius:14, padding:18, marginBottom:20, textAlign:"left" }}>
        <div style={{ fontSize:13, color:T.terra, lineHeight:1.7 }}>
          ...
          O teu pedido de adesão foi recebido e encontra-se em <strong>análise pela Direcção da AFAT</strong>.<br/><br/>
          Serás contactado no número <strong>{pedido.telefone}</strong>.<br/><br/>
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          <button onClick={() => onPartilhar("whatsapp")} style={{ background:"#25D366", color:T.branco, border:"none", borderRadius:12, padding:"13px 10px", fontWeight:900, fontSize:13, display:"flex", alignItems:"center", justifyContent:"center", gap:7 }}>WhatsApp</button>
          <button onClick={() => onPartilhar("facebook")} style={{ background:"#1877F2", color:T.branco, border:"none", borderRadius:12, padding:"13px 10px", fontWeight:900, fontSize:13, display:"flex", alignItems:"center", justifyContent:"center", gap:7 }}>Facebook</button>
        </div>
      </div>
      <button onClick={onNovoPedido} style={{ background:"none", color:T.muted, border:`1px solid ${T.areia}`, borderRadius:12, padding:"10px 20px", fontSize:13, fontWeight:700, width:"100%" }}>Registar outro membro</button>
    </div>
  );
}
...
function SeccaoPartilha() {
  const partilhar = useCallback((rede) => {
    ...
    const links = { whatsapp:`https://wa.me/?text=${texto}`, facebook:`https://www.facebook.com/sharer/sharer.php?u=${url}&quote=${texto}` };
  ...
  return (
    <section style={{ background:`linear-gradient(135deg,${T.terra},${T.ceuNoite})`, padding:"44px 22px", textAlign:"center", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ position:"relative", zIndex:1 }}>
        ...
        <TituloSeccao tag="Crescer Juntos" titulo="Convida a tua Família" subtitulo="Partilha este link com todos os Filhos e Amigos de Tambató — na Guiné e na Diáspora." centro claro />
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
          <button onClick={() => partilhar("whatsapp")} style={{ background:"#25D366", color:T.branco, border:"none", borderRadius:14, padding:"16px 12px", fontWeight:900, fontSize:15, display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>WhatsApp</button>
          <button onClick={() => partilhar("facebook")} style={{ background:"#1877F2", color:T.branco, border:"none", borderRadius:14, padding:"16px 12px", fontWeight:900, fontSize:15, display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>Facebook</button>
        </div>
        <button onClick={copiar} style={{ width:"100%", background:"rgba(255,255,255,.1)", color:T.branco, border:"1px solid rgba(255,255,255,.2)", borderRadius:14, padding:"13px", fontWeight:700, fontSize:14 }}>🔗 Copiar Link</button>
        <div style={{ marginTop:20, padding:"14px", background:"rgba(255,255,255,.06)", borderRadius:12, border:`1px solid rgba(255,255,255,.1)` }}>
          <p style={{ fontSize:12.5, color:"rgba(255,255,255,.55)", lineHeight:1.6, fontStyle:"italic", fontFamily:T.fontDisplay }}>"Somos mais fortes juntos. Cada membro que se junta é uma raiz que fortalece a nossa árvore."</p>
        </div>
      </div>
    </section>
  );
    ...
    <footer style={{ background:T.terra, padding:"28px 22px 20px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
        ...
        <div>
          ...
          <div onClick={onIrParaLogin} style={{ color:"rgba(255,255,255,.5)", fontSize:12, marginBottom:4, cursor:"pointer" }}>🔒 Área do Agente</div>
          <div onClick={onIrParaLogin} style={{ color:"rgba(255,255,255,.5)", fontSize:12, marginBottom:4, cursor:"pointer" }}>⚙️ Painel Admin</div>
          {["🪪 Registo de Sócio","📊 Estatísticas","📥 Exportação"].map(l => (
            <div key={l} style={{ color:"rgba(255,255,255,.5)", fontSize:12, marginBottom:4 }}>{l}</div>
          ))}
        </div>
      </div>
...
export default function PortalPublico({ onIrParaLogin }) {
      ...
      <div style={{ maxWidth:480, margin:"0 auto", boxShadow:"0 0 60px rgba(0,0,0,.12)", minHeight:"100vh", background:T.creme, position:"relative" }}>
        {vista === "sucesso" && pedidoSucesso && (
          <div style={{ padding:"28px 20px" }}>
            <EcranSucesso pedido={pedidoSucesso} onPartilhar={partilharRede} onNovoPedido={() => { setPedido(null); setVista("registo"); }} />
          </div>
        )}
        {vista !== "sucesso" && (
          <>
            <Hero onChamarRegisto={irParaRegisto} onIrParaLogin={onIrParaLogin} />
            <SobreTambato />
            <Divisor />
            <SeccaoInstitucional />
            <Divisor />
            <section id="registo" ref={registoRef} style={{ padding:"46px 22px 36px", background:T.creme }}>
              <TituloSeccao tag="Tornar Sócio" titulo="Junta-te à AFAT" subtitulo="Preenche o formulário abaixo. É gratuito e rápido. O teu Cartão de Sócio será emitido após aprovação da Direcção." />
              {vista === "registo" ? (
                ...
                <div style={{ background:`linear-gradient(135deg,${T.ouro}18,${T.areia}44)`, border:`2px solid ${T.ouro}55`, borderRadius:20, padding:26, textAlign:"center" }}>
                  ...
                  <h3 style={{ fontFamily:T.fontDisplay, fontSize:21, fontWeight:900, color:T.terra, marginBottom:8 }}>O teu Cartão de Sócio<br/>está a um passo</h3>
                  <p style={{ fontSize:13.5, color:T.muted, lineHeight:1.65, marginBottom:20 }}>Regista-te agora. O pedido entra automaticamente na fila de aprovação da Direcção da AFAT.</p>
                  <button onClick={irParaRegisto} style={{ background:`linear-gradient(135deg,${T.ouro},${T.ouroVivo})`, color:T.terra, border:"none", borderRadius:14, padding:"16px 32px", fontWeight:900, fontSize:15.5, boxShadow:`0 6px 24px ${T.ouro}50`, display:"inline-flex", alignItems:"center", gap:10 }}>🪪 Iniciar Registo</button>
                </div>
              )}
            </section>
            <SeccaoPartilha />
            <Rodape onIrParaLogin={onIrParaLogin} />
          </>
        )}
      </div>
    </>
